...
module.exports = function (options){
	...
	function getTableName(){	// Si l'attribut tableName n'existe pas dans le config, on prend le nom du module en minuscule
		if(_.isUndefined(modules[options])){
			throw new Error('Class : Module '+options+' does\'nt exist.');
		}
	...
	function localSave(onSuccess, onErrors){
		...
		if(this.isNew()){
			...
			sql = 'INSERT OR IGNORE INTO '+this.tableName+' ( '+field+' ) VALUES ( '+value+' )'; // tableName et idField dans le config du module
		}
		else{
			_.each(model, function(val, key){
				if(val !== null && val !== undefined){
		...
		var db = new LocalDb({name: app.params.localDbName});
