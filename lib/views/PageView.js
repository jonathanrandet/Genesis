...
function setModel(name){
	this.PageModel = name.toLowerCase();
}
...
function render(){
	...
	//	Display all data-templates
	this.$el.find('[data-template]').each(function(){
		var app = that.ParentView;
		var dataTemplate = $(this).data('template').split('-');
		app.getModule(dataTemplate[0]).getView(dataTemplate[1]).render();
		console.log('Bingo !!');
	});

	this.ParentView.CurrentPage = this.name;
}
