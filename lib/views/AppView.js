...
//var LocalDb = require('../models/LocalDb');
...
var Genesis = Backbone.View.extend({
	...
	stopAction: stopAction,
  	...
  	events: {
  		...
  		'click [data-submit]': 'submitAction', // submit
  		'click [type="submit"]': 'stopAction', // submit
...
function createStructure(){
	...
	_.each(pages,function(page){
		...
		if(!_.isUndefined(that.params) && !_.isUndefined(that.params.defaultModel)){
			newPage.setModel(that.params.defaultModel);
		}
		else{
		...
		that.addRoute(page.url, function(){   //$$$   ICI, TROUVER UN MOYEN de passer les params des l'url à params
			...
			var afterRenderAction = 'afterRender'+page.name;
			action = action.toLowerCase();
			afterRenderAction = afterRenderAction.toLowerCase();
			...
			// ICI AFTER RENDER
			if(_.isFunction(that.Controllers[page.name.toLowerCase()][afterRenderAction]) && !_.isUndefined(that.Pages[page.name])){
				that.Controllers[page.name.toLowerCase()][afterRenderAction](that.Pages[page.name], that);
			}
...
function afterRender(name, action){
	if(!_.isUndefined(name) && _.isString(name) && !_.isUndefined(action) && _.isFunction(action)){
		if(!_.isUndefined(this.Pages[name.toLowerCase()])){
			var actionName = 'afterRender'+name;
			actionName = actionName.toLowerCase();
			this[actionName] = action;		
		}
		else{
			throw new Error('afterRender : The page '+name+' doesn\'t exist');
		}
	}
	else{
		throw new Error('afterRender : The function need 2 parameters, pagename and action');
	}
}
...
function loadModules(){
	...
	_.each(modules, function(module, key){
		...
		if(!_.isUndefined(config) && !_.isUndefined(config.className)){
			...
			throw new Error('LoadModule : Module '+key+' need className');
		}
...
function createLocalDb(){
	var modules = templates.modules;
	var that = this;
	var sql = 'CREATE TABLE IF NOT EXISTS config (id_config INTEGER PRIMARY KEY, nom TEXT, valeur TEXT)';
	_.each(modules, function(module, key){
		var config = module.config;
		var table = module.config.tableName.toLowerCase();
		sql += table+' ( ';
		_.each(module.config.fields, function(field){
			var name 			= field.name;
			var type 			= field.type;
			var index			= field.index;
			var isNull 			= field.isNull;
			var defaultValue 	= field.defaultValue;
			if(_.isUndefined(name)){
				throw new Error('Bandi !!');
			}
			//sql += name.toLowerCase()+' '+type.toUpperCase()+' '+index.toUpperCase()+' ';
...
function submitAction(e) {
	...
	var formSelector = !_.isUndefined($btn.data('form')) ? $btn.data('form') : $btn.parent('form');

	if(this.$(formSelector).length > 0){
		var isValid = this.$(formSelector).valid(); // Verifier si cela ne pose pas de pb kan le form n'utilise pas Validate
		var model = new Backbone.Model();

		this.$(formSelector).each(function(){
			$(this).find('[name]').each(function(){
				if(!_.isUndefined($(this).val()) && !_.isNull($(this).val()) && $(this).val() !== ''){
					if($(this).attr('type') === 'checkbox' || $(this).attr('type') === 'radio'){
						model.set($(this).attr('name'), $(this).prop('checked'));
					}
					else{
		...
		if (_.isFunction(this.Controllers[this.CurrentPage.toLowerCase()][action])) {
			this.Controllers[this.CurrentPage.toLowerCase()][action](e, model, isValid);
		}
		else if(_.isFunction(this[action])){
			this[action](e, model, isValid);
		}
		else{
			console.error('data-submit : '+action+' is not a function');
		}

	}
	else{
		console.error('data-submit : you should specify a valid data-form');
	}
}
