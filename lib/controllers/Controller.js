...
function afterRender(action){
	if(!_.isUndefined(this.name) && _.isString(this.name) && !_.isUndefined(action) && _.isFunction(action)){
		if(!_.isUndefined(this.app.Pages[this.name.toLowerCase()])){
			var actionName = 'afterRender'+this.name;
			actionName = actionName.toLowerCase();
			this[actionName] = action;		
		}
		else{
			throw new Error('afterRender : The page '+this.name+' doesn\'t exist');
		}
	}
	else{
		throw new Error('afterRender : The function need action parameter.');
	}
}
